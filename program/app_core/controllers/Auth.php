...
class Auth extends CI_Controller {
    ...
    $data = curl_exec($ch);
    curl_close($ch);
     $countryCode = unserialize($data)['countryCode'];
  
        if($countryCode != 'IR'){
            die('آی پی شما خارج از ایران است. لطفا با آی پی ایران وارد شوید');
        }
        parent::__construct();
        $this->load->database();
            ...
            if ($this->form_validation->run() === TRUE)
            {
                ...
                if ($insert)
                {
                    ...
                    else
                    {
                        ...
                        $data = array('MerchantID' => '8346362e-48e4-11e9-9901-000c295eb8fc',
                            'Amount' => $rials/10,
